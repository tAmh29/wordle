...
var height = 6, width = 5, row = 0, col = 0;
var gameOver = false, isInputValid = true, gameStart = false;
var timerInterval = null, secondsElapsed = 0, shortestTime = Infinity;
var winstreak = 0;
...
function handleKeyboard() {
  ...
  keyboardButtons.forEach((key) => {
    let keyValue = key.innerText.toLowerCase();
    if (keyboardLetterStatus[keyValue] === "true") {
      key.style.backgroundColor = "#538D4E";
    } else if (keyboardLetterStatus[keyValue] === "present") {
      key.style.backgroundColor = "#B59F3B";
    } else if (keyboardLetterStatus[keyValue] === "absent") {
...
function handleWin() {
  if (secondsElapsed < shortestTime || shortestTime === Infinity) {
    ...
    if (typeof Storage !== "undefined") {
      localStorage.setItem("shortestTime", shortestTime.toString());
    }
    updateShortestTimeDisplay();
  }
  gameOver = true;
...
function resetKeyboard() {
  const keyboardButtons = document.querySelectorAll(".keyboard-button");
  keyboardButtons.forEach((key) => {
    key.style.backgroundColor = ""; // Set to your default button color
  });
}
...
function updateShortestTimeDisplay() {
  if (shortestTime === Infinity) {
    ...
    document.getElementById("shortest-time").innerText =
      "Shortest Time: " +
      (shortestMinutes < 10 ? "0" : "") +
      shortestMinutes +
      ":" +
      (shortestSeconds < 10 ? "0" : "") +
      shortestSeconds;
  }
}
...
function updateWinstreakDisplay() {
  let winStreakElement = document.getElementById("win-streak"); // Ensure you have an element with id="win-streak" in your HTML
  if (winStreakElement) {
    winStreakElement.innerText = "Win Streak: " + winstreak;
  }
}
